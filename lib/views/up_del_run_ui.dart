...
class UpDelRunUI extends StatefulWidget {
  ...
  // เอาตัวแปรที่สร้างมารับข้อมูลที่ส่งมาจากหน้าก่อนหน้านี่
  UpDelRunUI({
    super.key,
    this.runId,
    this.runLocation,
...
// ตอนหน้าจอถูกเปิดเเอาค่าที่ส่งมาจากหน้าก่อนหน้า มาแสดงที่ TexField ผ่านตัวควบคุม TexField
  @override
  void initState() {
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (runLocationCtrl.text.isEmpty) {
                    // แสดงไดอะล็อกคำเตือน
                    await _showWarningDialog('กรอกข้อมูลสถานที่วิ่งด้วย');
                  } else if (runDistanceCtrl.text.isEmpty) {
                    // แสดงไดอะล็อกคำเตือน
                    await _showWarningDialog('กรอกข้อมูลระยะทางที่วิ่งด้วย');
                  } else if (runTimeCtrl.text.isEmpty) {
                    // แสดงไดอะล็อกคำเตือน
                    await _showWarningDialog(
                        'กรอกข้อมูลเวลาที่ใช้ในการวิ่งด้วย');
                  } else {
                    ...
                    // ส่งข้อมูลโดยเอาข้อมูลที่จะส่งมาทำเป็น JSON
                    final result = await Dio().put(
                      'http://10.1.1.83:3030/api/run/${widget.runId}',
                      data: run.toJson(),
                    );
                    ...
                    if (result.statusCode == 200) {
                      ...
                      await _showWarningDialog(
                          'ไม่สามารถแก้ไขบันทึกการวิ่งได้');
                    }
                  }
                },
                child: Text(
